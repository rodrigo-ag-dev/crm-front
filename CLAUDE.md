...
- `components/FinancialPaymentModal.tsx` — payoff modal, reused for both single and batch payoff. Branches on `installments.length === 1`: single payoff lets you edit `amountPaid` (defaults to the installment's `amount`, supports partial/adjusted payment) and calls `POST /financial/installments/{id}/pay`; batch (from the table's multi-select) has no per-row `amountPaid` field and calls `POST /financial/installments/pay-batch` with the full `installmentIds` list, matching what the backend DTO accepts.
